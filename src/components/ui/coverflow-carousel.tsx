...
export function CoverflowCarousel({
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const clickedIndex = slide ? parseInt(slide.getAttribute('data-slide-index') || "", 10) : undefined;
    ...
    dragRef.current = {
      ...
      v: 0,
      t: performance.now(),
      clickedIndex: isNaN(clickedIndex as number) ? undefined : clickedIndex,
    };
  };
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const draggedDist = Math.abs(drag.x - event.clientX);
    if (draggedDist < 5 && drag.clickedIndex !== undefined) {
      goTo(drag.clickedIndex);
      return;
    }
            ...
            {slides.map((slide, index) => (
              ...
              >
                ...
                <img
                  ...
                  alt={slide.alt}
                  draggable={false}
                  className="h-full w-full select-none object-cover bg-gray-100"
                />
              </div>
            ))}
          </div>
